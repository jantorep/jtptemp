...
$rdsbrokers = "$config.ConnectionBroker01", "$config.ConnectionBroker02"
$rdshosts = "$config.RDSHost01", "$config.RDSHost02"
$rdsgateways = "$config.RDGatewayServer01", "$config.RDGatewayServer02"
$rdswebsrvs = "$config.WebAccessServer01", "$config.WebAccessServer02"
...
#Install Features !!!!!!!!!!!!!!!! NEEEDS UPDATING!!!!!!!!!!!!!!!!!!!!!
Add-WindowsFeature –Name RDS-Web-Access –IncludeAllSubFeature -ComputerName $rdswebsrvs
Add-WindowsFeature –Name RDS-Gateway –IncludeAllSubFeature -ComputerName $rdsgateways
Add-WindowsFeature –Name RDS-RD-Server –IncludeAllSubFeature -ComputerName $rdshosts
Add-WindowsFeature –Name RDS-Connection-Broker  –IncludeAllSubFeature -ComputerName $rdsbrokers -Restart
...
#Install RDH TML5 web client######

Invoke-Command -ComputerName $config.WebAccessServer01 -ScriptBlock {
...
Invoke-Command -ComputerName $config.WebAccessServer02 -ScriptBlock {
    Enter-PSSession -ComputerName $rdswebsrvname
    Install-Module -Name PowerShellGet -Force
    Install-Module -Name RDWebClientManagement
    Install-RDWebClientPackage
